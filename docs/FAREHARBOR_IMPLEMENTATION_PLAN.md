...
The current increment keeps GHL staff synchronization and optional operational role labels, but deliberately does not use staff coverage as an availability or booking constraint. It also adds resource type/capacity/notes metadata and migration support, upgrades the manual booking drawer to show every valid slot for the selected date, adds customer-type/rate selection and rate-specific quantity limits, and keeps the booking engine authoritative for hours, blocks, cutoff rules, and inventory.
