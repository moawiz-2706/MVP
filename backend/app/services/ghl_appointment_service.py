...
class GHLAppointmentService:
    ...
    def _appointment_description(
        ...
        lines = [
            ...
            f"Customer type: {booking.customer_type_name_snapshot or 'Legacy calendar unit'}",
            f"Seats: {booking.seat_count}",
            f"Unit price: {order.currency.upper()} {booking.base_price_minor / 100:,.2f}",
            f"Subtotal: {order.currency.upper()} {booking.line_subtotal_minor / 100:,.2f}",
            f"Booking fee: {order.currency.upper()} {booking.booking_fee_minor / 100:,.2f}",
            f"Tax: {order.currency.upper()} {booking.tax_minor / 100:,.2f}",
            f"Line total: {order.currency.upper()} {booking.line_total_minor / 100:,.2f}",
            f"Payment status: {self.db.scalar(select(Payment.status).where(Payment.booking_order_id == order.id)) or 'unknown'}",
