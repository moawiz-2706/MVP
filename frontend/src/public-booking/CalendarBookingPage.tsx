...
export function CalendarBookingPage() {
  ...
  const create = useMutation({
    mutationFn: () => api<Created>(`/public/${operatorSlug}/orders`, { ...json("POST", { items: requestItems, customer: { first_name: first, last_name: last, email, phone: phone || null }, custom_fields: customFields }), headers: { "X-Checkout-Key": checkoutKey } }),
    onSuccess: (result) => {
      setCreated(result);
  ...
  return <PublicFrame name={catalog.data.name} embed={embed}>
    ...
    {created?.client_secret ? <div className="public-card" style={{ maxWidth: 620 }}><h2>Complete secure payment</h2><PaymentPanel clientSecret={created.client_secret} publicReference={created.public_reference} accessToken={created.access_token} embed={embed} /></div> : <div className="booking-panel">
      <section className="public-card"><h2>Choose a date and time</h2>
        ...
        {slot && <div style={{ marginTop: 20, paddingTop: 17, borderTop: "1px solid #e6e9e8" }}>{rates.data?.length ? <label className="field" style={{ marginBottom: 12 }}><span>Booking type</span><select required value={selectedRateId} onChange={(event) => { setSelectedRateId(event.target.value); setUnits(1); }}><option value="">Choose kayak type</option>{rates.data.map((rate) => <option key={rate.id} value={rate.id}>{rate.customer_type_name} · {formatMoney(rate.price_minor, calendar.currency)} · {rate.seat_count} seat{rate.seat_count === 1 ? "" : "s"}</option>)}</select>{selectedRate?.note && <small>{selectedRate.note}</small>}</label> : null}<div className="summary-row"><span>Quantity</span><div className="quantity"><button onClick={() => setUnits(Math.max(1, units - 1))}><Minus size={14} /></button><strong>{units}</strong><button onClick={() => setUnits(Math.min(maxAllowedUnits, units + 1))}><Plus size={14} /></button></div></div><div className="summary-row"><span>Base price</span><strong>{formatMoney(selectedRate?.price_minor ?? calendar.base_price_minor, calendar.currency)} each</strong></div><button className="button" style={{ width: "100%", marginTop: 12 }} disabled={Boolean(rates.data?.length && (!selectedRate || maxAllowedUnits < 1))} onClick={add}><ShoppingBag size={15} />Add booking</button></div>}
      </section>
      <aside className="public-card"><h2>Your booking</h2>{cart.length === 0 ? <p style={{ fontSize: 12, color: "#78827f" }}>Select a time to begin.</p> : <>{cart.map((item, index) => <div key={`${item.start_at}-${index}`} style={{ padding: "10px 0", borderBottom: "1px solid #edf0ef", display: "flex", justifyContent: "space-between", gap: 10 }}><div className="cell-title"><strong>{formatDay(item.start_at, selectedTimeZone)} · {formatTime(item.start_at, selectedTimeZone)}</strong><span>{item.units} × {item.rate?.customer_type_name || calendar.name}</span></div><button className="icon-button" onClick={() => setCart(cart.filter((_, itemIndex) => itemIndex !== index))}><Trash2 size={14} /></button></div>)}{quote.data && <div style={{ marginTop: 10 }}><div className="summary-row"><span>Subtotal</span><span>{formatMoney(quote.data.subtotal_minor, quote.data.currency)}</span></div><div className="summary-row"><span>Booking fees</span><span>{formatMoney(quote.data.booking_fee_minor ?? quote.data.platform_fee_and_taxes_minor, quote.data.currency)}</span></div>{quote.data.tax_minor ? <div className="summary-row"><span>Taxes</span><span>{formatMoney(quote.data.tax_minor, quote.data.currency)}</span></div> : null}<div className="summary-row total"><span>Total</span><span>{formatMoney(quote.data.customer_total_minor, quote.data.currency)}</span></div></div>}<form onSubmit={(event: FormEvent) => { event.preventDefault(); create.mutate(); }} style={{ marginTop: 17 }}><div className="form-grid"><label className="field"><span>First name</span><input required value={first} onChange={(event) => setFirst(event.target.value)} /></label><label className="field"><span>Last name</span><input required value={last} onChange={(event) => setLast(event.target.value)} /></label><label className="field full"><span>Email</span><input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label><label className="field full"><span>Phone (optional)</span><input value={phone} onChange={(event) => setPhone(event.target.value)} /></label>{publicFields.data?.map((field) => <label className="field full" key={field.id}><span>{field.label}{field.required ? " *" : ""}</span>{field.field_type === "textarea" ? <textarea required={field.required} value={String(customFields[field.key] || "")} onChange={(event) => setCustomFields({ ...customFields, [field.key]: event.target.value })} /> : field.field_type === "select" ? <select required={field.required} value={String(customFields[field.key] || "")} onChange={(event) => setCustomFields({ ...customFields, [field.key]: event.target.value })}><option value="">Choose…</option>{(field.options || []).map((option) => <option key={String(option)} value={String(option)}>{String(option)}</option>)}</select> : field.field_type === "boolean" ? <input type="checkbox" checked={Boolean(customFields[field.key])} onChange={(event) => setCustomFields({ ...customFields, [field.key]: event.target.checked })} /> : <input required={field.required} type={field.field_type === "number" ? "number" : field.field_type === "date" ? "date" : "text"} value={String(customFields[field.key] || "")} onChange={(event) => setCustomFields({ ...customFields, [field.key]: event.target.value })} />}</label>)}</div>{create.error && <div className="error-banner" style={{ marginTop: 12 }}>{create.error.message}</div>}<button className="button" style={{ width: "100%", marginTop: 15 }} disabled={create.isPending || quote.isLoading}>{create.isPending ? "Reserving inventory…" : "Continue to payment"}</button></form></>}</aside>
    </div>}
  </PublicFrame>;
}
