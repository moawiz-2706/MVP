...
export function CalendarBookingPage() {
  ...
  useEffect(() => {
    if (catalog.data && !day) setDay(tomorrowInZone(defaultTimeZone));
  }, [catalog.data, defaultTimeZone, day]);

  const availability = useQuery({
    queryKey: ["availability", operatorSlug, calendarSlug, day],
    queryFn: () => api<AvailabilityResponse>(`/public/${operatorSlug}/calendars/${calendarSlug}/availability?${new URLSearchParams({ date: day })}`),
    enabled: Boolean(calendar && day && calendar.public_booking_mode === "online"),
    staleTime: 0,
    gcTime: 60_000,
  ...
  // Availability is polled while this page is open. Always resolve the selected
  // slot from the newest response so inventory changes are reflected without a
  // manual refresh or a stale quantity limit.
  const liveSlot = slot ? availability.data?.slots.find((item) => item.start_at === slot.start_at) || slot : null;
  const selectedRateAvailability = liveSlot?.rates?.find((rate) => rate.rate_id === selectedRateId);
  const maxAllowedUnits = selectedRateAvailability?.available_quantity ?? liveSlot?.max_bookable_units ?? 1;
  ...
  const add = () => {
    if (!liveSlot || calendar.public_booking_mode !== "online") return;
    if (rates.data?.length && (!selectedRate || units > maxAllowedUnits)) return;
    setCart([...cart, { calendar_id: calendar.id, start_at: liveSlot.start_at, units, slot: liveSlot, rate_id: selectedRate?.id, rate: selectedRate || undefined }]);
    setSlot(null);
    setUnits(1);
  };
  const selectSlot = async (item: AvailabilitySlot) => {
    const refreshed = await availability.refetch();
    setSlot(refreshed.data?.slots.find((candidate) => candidate.start_at === item.start_at) || item);
    setUnits(1);
  };
  const policy = calendar.booking_policy;
  ...
  return <PublicFrame name={catalog.data.name} embed={embed}>
    ...
    <div className="tour-detail-hero">{calendar.hero_image_url ? <img src={calendar.hero_image_url} alt={calendar.name} /> : <div className="tour-detail-fallback">{calendar.name.slice(0, 1)}</div>}<div className="tour-detail-copy"><span className="eyebrow">{calendar.category_name || "Online booking"}</span><h1>{calendar.name}</h1><p className="tour-headline">{calendar.headline || calendar.description}</p><div className="service-meta" style={{ marginTop: 15, display: "flex", gap: 18 }}><span><Clock3 size={15} />{calendar.duration_minutes} minutes</span>{calendar.location && <span><MapPin size={15} />{calendar.location.name} · {calendar.location.address}</span>}</div></div></div>
    {calendar.gallery_image_urls?.length ? <div className="tour-gallery">{calendar.gallery_image_urls.map((image, index) => <img key={`${image}-${index}`} src={image} alt="" loading="lazy" />)}</div> : null}
    {calendar.description && calendar.headline && <div className="public-card tour-description"><h2>About this experience</h2><p>{calendar.description}</p>{calendar.booking_instructions && <p>{calendar.booking_instructions}</p>}</div>}
    {calendar.public_booking_mode === "call_to_book" ? <div className="public-card booking-callout"><h2>Call to book</h2><p>This experience is available by phone so the operator can confirm the right equipment and schedule.</p>{calendar.call_to_book_phone && <a className="button" href={`tel:${calendar.call_to_book_phone}`}>Call {calendar.call_to_book_phone}</a>}</div> : calendar.public_booking_mode === "closed" ? <div className="public-card booking-callout"><h2>Online booking is closed</h2><p>Please contact the operator for the next available booking window.</p>{calendar.call_to_book_phone && <a className="button" href={`tel:${calendar.call_to_book_phone}`}>Call the operator</a>}</div> : created?.client_secret ? <div className="public-card" style={{ maxWidth: 620 }}><h2>Payment details</h2><p style={{ color: "#78827f", fontSize: 13, display: "flex", gap: 7, alignItems: "center" }}><ShieldCheck size={15} />Secured and encrypted payment</p><PaymentPanel clientSecret={created.client_secret} publicReference={created.public_reference} accessToken={created.access_token} embed={embed} /></div> : <div className="booking-panel">
      <section className="public-card"><h2>Choose a date and time</h2>
        ...
        <div className="operator-timezone-note">Times shown in the operator's local timezone ({defaultTimeZone}).</div><label className="field" style={{ maxWidth: 220, marginBottom: 17 }}><span>Choose a date</span><input type="date" min={tomorrowInZone(defaultTimeZone)} value={day} onChange={(event) => { setDay(event.target.value); setSlot(null); }} /></label>
        {availability.isLoading ? <div className="loading">Checking live availability…</div> : availability.error ? <div className="error-banner">{availability.error.message}</div> : <div className="slot-grid">{availability.data?.slots.map((item) => <button key={item.start_at} disabled={!item.available || availability.isFetching} className={`slot ${slot?.start_at === item.start_at ? "selected" : ""}`} onClick={() => { void selectSlot(item); }}>{formatTime(item.start_at, defaultTimeZone)}{item.available && item.rates?.length ? <span className="slot-rate-summary">{item.rates.map((rate) => <span key={rate.rate_id}>{rate.customer_type_name}: {rate.available_quantity} available</span>)}</span> : <span>{item.available ? `${item.max_bookable_units} available` : item.status === "call_to_book" ? "Call to book" : item.status === "past_cutoff" ? "Booking closed" : item.status === "closed" ? "Closed" : item.status === "blocked" ? "Unavailable" : "Sold out"}</span>}</button>)}</div>}
