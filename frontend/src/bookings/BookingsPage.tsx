...
import { formatLongDate, formatTime, isValidDateTime, zoneLabel, zonedWallTimeToISO } from "../lib/datetime";
...
function bookingRequestError(error: unknown, endpoint: string): string | null {
  if (!error) return null;
  if (error instanceof ApiError) {
    if (error.status === 401 || error.status === 403) return `${endpoint} failed with HTTP ${error.status}. Your HighLevel session or booking permission has expired; reload the embedded app and sign in again.`;
    if (error.status >= 500) return `${endpoint} failed with HTTP ${error.status}. This usually means the Supabase schema is behind the deployed Passport code. Apply migrations 024 and 025, then redeploy the backend.`;
    return `${endpoint} failed with HTTP ${error.status}: ${error.message}`;
  }
  return `${endpoint} failed: ${error instanceof Error ? error.message : "Network error"}`;
}
...
function BookingDrawer({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  const [weatherReason, setWeatherReason] = useState("Weather closure");
  ...
  const statusMutation = useMutation({ mutationFn: (status: "completed" | "no_show") => api<BookingDetail>(`/bookings/${id}/status`, json("POST", { status, reason: status === "no_show" ? "Marked no-show by operator" : "Marked completed by operator" })), onSuccess: async () => { await Promise.all([client.invalidateQueries({ queryKey: ["bookings"] }), client.invalidateQueries({ queryKey: ["booking", id] })]); } });
  const weatherMutation = useMutation({ mutationFn: () => api(`/bookings/${id}/weather-cancel`, json("POST", { calendar_id: data!.calendar_id, start_at: data!.start_at, end_at: data!.end_at, reason: weatherReason, refund_mode: "full_refund" })), onSuccess: async () => { setWeatherOpen(false); await Promise.all([client.invalidateQueries({ queryKey: ["bookings"] }), client.invalidateQueries({ queryKey: ["booking", id] }), client.invalidateQueries({ queryKey: ["booking-notifications"] })]); } });
      ...
      <aside className="drawer" aria-label="Booking details">
        {isLoading || !data ? <div className="loading">Loading booking...</div> : (
          <>
            ...
            <div className="detail-section"><h3>Payment</h3><dl className="detail-list"><dt>Status</dt><dd><span className={`badge ${badge(data.payment_status)}`}>{data.payment_status}</span></dd><dt>Subtotal</dt><dd>{money(data.subtotal_minor)}</dd><dt>Booking fees</dt><dd>{money(data.booking_fee_minor)}</dd><dt>Taxes</dt><dd>{money(data.tax_minor)}</dd><dt>Platform total</dt><dd>{money(data.platform_fee_and_taxes_minor)}</dd><dt>Total</dt><dd>{money(data.customer_total_minor)}</dd></dl></div>
            ...
            {weatherOpen && <div className="inline-editor"><label className="field"><span>Weather closure reason</span><input value={weatherReason} onChange={event => setWeatherReason(event.target.value)} /></label><div className="toolbar"><button className="button ghost small" onClick={() => setWeatherOpen(false)}>Keep booking</button><button className="button danger small" disabled={!weatherReason.trim() || weatherMutation.isPending} onClick={() => weatherMutation.mutate()}>{weatherMutation.isPending ? "Closing…" : "Close and refund"}</button></div></div>}
            {statusMutation.error && <div className="error-banner">{statusMutation.error.message}</div>}{weatherMutation.error && <div className="error-banner">{weatherMutation.error.message}</div>}
            <div className="toolbar"><button className="button secondary" disabled={data.status === "cancelled" || cancel.isPending} onClick={() => cancel.mutate()}>Cancel booking</button><button className="button secondary" disabled={!(["confirmed", "pending_payment"].includes(data.status)) || statusMutation.isPending} onClick={() => statusMutation.mutate("no_show")}>Mark no-show</button><button className="button secondary" disabled={data.status !== "confirmed" || statusMutation.isPending} onClick={() => statusMutation.mutate("completed")}>Mark completed</button><button className="button ghost small" disabled={data.status === "cancelled" || weatherMutation.isPending} onClick={() => setWeatherOpen(true)}>Weather closure</button>{(data.ghl_contact_sync_status === "failed" || data.ghl_confirmation_email_status === "failed" || data.ghl_appointment_sync_status === "failed" || data.ghl_appointment_sync_status === "dead") && <button className="button secondary" disabled={retry.isPending} onClick={() => retry.mutate()}>{retry.isPending ? "Retrying…" : "Retry GHL sync"}</button>}</div>
          </>
        )}
      </aside>
    </>
    ...
    <>
      ...
      <div className="detail-section" style={{ marginBottom: 0 }}>
          ...
          <div className="slot-list">
            {entry.staff.map((member) => (
              <div key={member.id} className="staff-row staff-row-assignment">
                ...
                <select className="control" aria-label={`Staff for ${member.role || "booking"}`} value={member.staff_id} disabled={reassign.isPending} onChange={(e) => reassign.mutate({ id: member.id, staffId: e.target.value, role: member.role })}>{(candidates.data || []).filter((candidate) => matchesRole(candidate, member.role)).map((candidate) => <option key={candidate.staff_id} value={candidate.staff_id} disabled={!candidate.available && candidate.staff_id !== member.staff_id}>{candidate.available ? candidate.name : `${candidate.name} — unavailable`}</option>)}</select><select key={`${member.id}-${member.role ?? ""}`} className="control" aria-label={`Role for ${member.staff_name}`} value={member.role || ""} onChange={(e) => rename.mutate({ id: member.id, value: e.target.value })}><option value="">Choose role</option>{roleOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select>
        ...
        <form className="assign-form" onSubmit={(e) => { e.preventDefault(); assign.mutate(); }}>
          ...
          <select className="control" required aria-label="Required staff role" value={role} onChange={(e) => { setRole(e.target.value); setStaffId(""); }}><option value="">Choose role</option>{roleOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select>
          <button className="button" disabled={!staffId || assign.isPending}><UserPlus size={15} />{assign.isPending ? "Assigning…" : "Assign"}</button>
        </form>
        {candidates.data?.length === 0 && <p className="muted-note">Sync GHL staff and assign custom roles on the Staff page before assigning them here.</p>}
...
function ConfigureSlotForm({ calendars, initialStart, onDone }: { calendars: Calendar[]; initialStart?: string; onDone: () => void }) {
  ...
  const roleOptions = Array.from(new Set((candidates.data || []).map((candidate) => candidate.custom_role))).filter((option): option is string => Boolean(option)).sort();
  const options = (candidates.data || []).filter((candidate) => candidate.available && candidate.custom_role?.toLowerCase() === role.trim().toLowerCase());
  return <form onSubmit={(event) => { event.preventDefault(); assign.mutate(); }}><p className="muted-note" style={{ marginTop: 0 }}>Choose a Passport custom role first. Only GHL staff with that role can be assigned to this slot.</p><div className="form-grid"><label className="field full"><span>Calendar</span><select required value={calendarId} onChange={(event) => { setCalendarId(event.target.value); setStaffId(""); }}><option value="">Choose a calendar</option>{calendars.map((calendar) => <option key={calendar.id} value={calendar.id}>{calendar.name}</option>)}</select></label><label className="field full"><span>Slot start ({zoneLabel(me.operator.time_zone)})</span><input type="datetime-local" required value={start} onChange={(event) => { setStart(event.target.value); setStaffId(""); }} /></label><label className="field"><span>Required staff role</span><select required value={role} onChange={(event) => { setRole(event.target.value); setStaffId(""); }} disabled={!start || candidates.isLoading}><option value="">Choose role</option>{roleOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select></label><label className="field"><span>Matching staff member</span><select required value={staffId} onChange={(event) => setStaffId(event.target.value)} disabled={!start || candidates.isLoading || !role}><option value="">{candidates.isLoading ? "Checking availability…" : options.length ? "Choose staff" : "No available matching staff"}</option>{options.map((candidate) => <option key={candidate.staff_id} value={candidate.staff_id}>{candidate.name}</option>)}</select></label></div>{candidates.error && <div className="error-banner" style={{ marginTop: 12 }}>{candidates.error.message}</div>}{assign.error && <div className="error-banner" style={{ marginTop: 12 }}>{assign.error.message}</div>}<div className="dialog-actions"><button type="button" className="button secondary" onClick={onDone}>Cancel</button><button className="button" disabled={!staffId || assign.isPending}>{assign.isPending ? "Assigning…" : "Configure slot"}</button></div></form>;
}
...
export function BookingsPage() {
  ...
  const eventData = useMemo(() => (slots.data || []).flatMap((slot) => slot.calendars.filter((entry) => isValidDateTime(slot.start_at) && isValidDateTime(entry.end_at)).map((entry) => ({
    id: `${entry.calendar_id}|${slot.start_at}`,
    start: slot.start_at,
    end: entry.end_at,
    backgroundColor: `${entry.category_color || "#c98945"}22`,
    borderColor: entry.category_color || "#c98945",
    textColor: "#4b3421",
    extendedProps: { slot, entry },
  }))), [slots.data]);
  const eventContent = (arg: EventContentArg) => {
    const slot = arg.event.extendedProps.slot as DashboardSlot;
    const entry = arg.event.extendedProps.entry as DashboardSlot["calendars"][number];
    const booking = entry.bookings.find(holdsSeat);
    const available = entry.capacity === null ? null : Math.max(entry.capacity - entry.booked_units, 0);
    return <div className="event-compact" title={`${entry.calendar_name} · ${plural(entry.bookings.filter(holdsSeat).length, "booking")}`}><div className="event-title"><span className="event-lock">{entry.staff.length ? "▣" : "□"}</span><time>{formatTime(slot.start_at, tz)}</time> {entry.calendar_name}</div>{booking && <div className="event-customer">{booking.customer_name}</div>}<div className="event-meta"><span>▣ {entry.booked_units}</span>{available !== null && <span>◻ {available}</span>}</div></div>;
  };
