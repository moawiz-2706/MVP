...
import type { Booking, BookingDetail, BookingNote, Calendar, Category, DashboardSlot, SlotCalendar, StaffAssignment, StaffCandidate } from "../api/types";
...
function BookingDrawer({ id, onClose }: { id: string; onClose: () => void }) {
  ...
  const cancel = useMutation({ mutationFn: () => api<void>(`/bookings/${id}/cancel`, { method: "POST" }), onSuccess: async () => { await client.invalidateQueries({ queryKey: ["bookings"] }); await client.invalidateQueries({ queryKey: ["booking", id] }); } });
      ...
      <aside className="drawer" aria-label="Booking details">
        {isLoading || !data ? <div className="loading">Loading booking...</div> : (
          <>
            ...
            <div className="detail-section"><h3>GoHighLevel</h3><dl className="detail-list"><dt>Contact sync</dt><dd>{data.ghl_contact_sync_status}</dd><dt>Confirmation</dt><dd>{data.ghl_confirmation_email_status}</dd></dl></div>
            <div className="toolbar"><button className="button secondary" disabled={data.status === "cancelled" || cancel.isPending} onClick={() => cancel.mutate()}>Cancel booking</button>{(data.ghl_contact_sync_status === "failed" || data.ghl_confirmation_email_status === "failed") && <button className="button secondary" onClick={() => retry.mutate()}>Retry GHL sync</button>}</div>
          </>
        )}
      </aside>
    </>
...
function SlotCalendarDetail({ entry, startAt, onBack, onOpenBooking }: { entry: SlotCalendar; startAt: string; onBack: () => void; onOpenBooking: (id: string) => void }) {
  ...
  const refresh = () => Promise.all([client.invalidateQueries({ queryKey: ["bookings"] }), client.invalidateQueries({ queryKey: ["staff-candidates"] }), client.invalidateQueries({ queryKey: ["staff"] })]);
          ...
          <div className="slot-list">
            {entry.staff.map((member) => (
              <div key={member.id} className="staff-row">
                ...
                <input key={`${member.id}-${member.role ?? ""}`} className="control" aria-label={`Role for ${member.staff_name}`} defaultValue={member.role || ""} placeholder="Role" list="staff-roles" maxLength={80} onBlur={(e) => { const value = e.target.value.trim(); if (value !== (member.role || "")) rename.mutate({ id: member.id, value }); }} />
