...
import { ArrowLeft, CalendarPlus, Check, ChevronRight, Copy, ExternalLink, Search, Trash2, UserPlus, X } from "lucide-react";
import { useEffect, useMemo, useState, type FormEvent } from "react";
import { useLocation } from "react-router-dom";
import { api, ApiError, json } from "../api/client";
import type { Booking, BookingDetail, BookingNote, BookingParticipant, Calendar, Category, DashboardSlot, SlotCalendar, StaffAssignment, StaffCandidate } from "../api/types";
    ...
    <>
      ...
      <div className="detail-section" style={{ marginBottom: 0 }}>
        ...
        <form className="assign-form" onSubmit={(e) => { e.preventDefault(); assign.mutate(); }}>
          ...
          <select className="control" aria-label="Optional staff role" value={role} onChange={(e) => { setRole(e.target.value); setStaffId(""); }}><option value="">Optional role</option>{roleOptions.map((option) => <option key={option} value={option}>{option}</option>)}</select>
...
export function BookingsPage() {
  ...
  const [filter, setFilter] = useState<{type:"all"|"category"|"calendar"; id?:string}>({type:"all"}); const [search, setSearch] = useState(""); const [selected, setSelected] = useState<string | null>(null); const [creating, setCreating] = useState(false);
